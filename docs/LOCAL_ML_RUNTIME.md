...
The current STT path is file-based. Use `jarvis-codex voice discover --json` to find local `whisper-cli` and ggml model candidates without processing audio or writing state. Use `jarvis-codex voice probe --audio-file ... --model ... --stt-command ... --json` to verify local adapter readiness without processing audio or writing state. Use `jarvis-codex voice ingest --audio-file ... --model ... --stt-command ... --allow-audio-processing --json` only when the operator approves one local audio-processing action. The adapter is a local executable that receives `--audio-file` and `--model`, writes the transcript to stdout, and exits. Jarvis includes `scripts/whisper-cpp-stt-adapter.py` for local `whisper.cpp` usage, including `--check-only` readiness checks. Jarvis does not download models, start listeners, access microphones, or choose a hidden cloud fallback.
