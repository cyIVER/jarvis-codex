...
The current STT path is file-based: `jarvis-codex voice ingest --audio-file ... --model ... --stt-command ... --allow-audio-processing --json`. The adapter is a local executable that receives `--audio-file` and `--model`, writes the transcript to stdout, and exits. Jarvis includes `scripts/whisper-cpp-stt-adapter.py` for local `whisper.cpp` usage. Jarvis does not download models, start listeners, access microphones, or choose a hidden cloud fallback.
