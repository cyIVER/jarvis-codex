...
| Voice ingress can start without always-on listeners. | Medium | Transcript capture exists, `voice probe --audio-file ... --model ... --stt-command ... --json` checks local STT readiness without runtime work, `voice ingest --audio-file ... --model ... --stt-command ... --allow-audio-processing --json` supports explicit local STT adapter execution, and `scripts/whisper-cpp-stt-adapter.py` wraps local `whisper.cpp`. | Exercise transcription against an operator-selected `whisper-cli` binary and local ggml model; keep microphone and Codex App Server bridges separate. |
