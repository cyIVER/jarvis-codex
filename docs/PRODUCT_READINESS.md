...
| Voice ingress can start without always-on listeners. | Medium | Transcript capture exists, `voice discover --json` finds local STT assets without writes, `voice probe --audio-file ... --model ... --stt-command ... --json` checks local STT readiness without runtime work, `voice ingest --audio-file ... --model ... --stt-command ... --allow-audio-processing --json` supports explicit local STT adapter execution, `scripts/whisper-cpp-stt-adapter.py` wraps local `whisper.cpp`, and `docs/WHISPER_CPP_STT_RUNBOOK.md` documents operator setup. | Exercise transcription against an operator-selected `whisper-cli` binary and local ggml model; keep microphone and Codex App Server bridges separate. |
...
- Whether the next package should exercise real local STT transcription with an operator-selected adapter/model, start release publication planning, or broaden release readiness review.
