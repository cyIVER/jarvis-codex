...
export function buildDisplayEntries(
  ...
  // Do not attach a trailing persisted tool-only turn to the previous text
  // assistant message. Those tool events belong to an incomplete/hidden turn and
  // should remain hidden rather than making the previous answer look active.
...
export function getTrailingToolOnlyTurnSummary(
  ...
  for (const message of trailing) {
    if (isAssistantToolCallOnlyMessage(message)) {
      count += 1
      for (const call of getToolCallsFromMessage(message)) toolNames.add(call.name)
      continue
    }
    if (message.role === 'tool' || message.role === 'toolResult') {
      count += 1
  ...
  return count > 0
    ? { count, toolNames: [...toolNames].sort(), hasFinalAssistantText: true }
    : null
}
